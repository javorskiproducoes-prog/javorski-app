...
import { EQUIPE, CATEGORIAS, TEMPOS, STATUS_OPTIONS, TURNOS, categoriaShowsE, categoriaShowsS, corLeve } from '../lib/constants'
import { Avatar } from '../components/Avatar'
import { showToast } from '../components/Toast'

function itemVazio() {
  return { cat: '', ev: '', tm: '', st: '', ob: '', ed: '', se: '' }
}
...
export function Registrar({ salvarRegistro }) {
  ...
  async function salvar() {
    ...
    const payload = {
      ...
      data: form.date,
      turno: form.turno,
      blocos: form.blocos.map(b => b.filter(i => i.cat || i.ev)),
    }
  ...
  if (!pessoaSel) {
    return (
      <div>
        ...
        <p className="text-muted mb-4" style={{ fontSize: 13 }}>Selecione um colaborador para registrar o dia:</p>
        <div className="grid-pessoas">
          {EQUIPE.map(p => (
            <button key={p.nome} className="pessoa-card" onClick={() => selecionarPessoa(p)}>
              ...
              <span className="tipo-badge">{p.tipo === 'video' ? 'Vídeo' : p.tipo === 'foto' ? 'Foto' : 'Adm/Mkt'}</span>
            </button>
          ))}
        </div>
      </div>
    ...
    <div>
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <div className="text-xs text-muted">{pessoaSel.tipo === 'video' ? 'Editor de Vídeo' : pessoaSel.tipo === 'foto' ? 'Fotógrafo/Editor' : 'Administrativo/Marketing'}</div>
        </div>
        <div className="flex gap-3 items-center" style={{ marginLeft: 'auto' }}>
          <div>
            <label className="form-label">Data</label>
            <input type="date" className="form-input" style={{ width: 160 }} value={form.date} onChange={e => setField('date', e.target.value)} />
          </div>
          <div>
            <label className="form-label">Turno</label>
            <select className="form-select" style={{ width: 160 }} value={form.turno} onChange={e => setField('turno', e.target.value)}>
        ...
        {form.blocos.map((bloco, bi) => (
          <div key={bi} className="bloco">
            <div className="bloco-header" style={{ borderLeft: `3px solid ${pessoaSel.cor}` }}>
              ...
              <span style={{ color: pessoaSel.cor }}>{bloco.filter(i => i.cat || i.ev).length} item(s)</span>
            </div>

            {bloco.map((item, ii) => (
              <div key={ii} className="bloco-item" style={{ borderLeftColor: pessoaSel.cor }}>
                {/* Row 1: categoria + projeto */}
                <div className="bloco-row cols2 mb-2">
                  ...
                  <div>
                    ...
                    <input className="form-input" value={item.ev} onChange={e => setItemField(bi, ii, 'ev', e.target.value)} placeholder="Ex: Bianca e Leonardo" />
                  </div>
                </div>

                {/* Row 2: tempo + status + E + S */}
                <div className={`bloco-row mb-2 ${categoriaShowsE(item.cat) && categoriaShowsS(item.cat) ? 'cols5' : categoriaShowsE(item.cat) || categoriaShowsS(item.cat) ? 'cols4' : 'cols3'}`}>
                  <div>
                    ...
                    <select className="form-select" value={item.tm} onChange={e => setItemField(bi, ii, 'tm', e.target.value)}>
                      <option value="">—</option>
                  ...
                  <div>
                    ...
                    <select className="form-select" value={item.st} onChange={e => setItemField(bi, ii, 'st', e.target.value)}>
                  ...
                  {categoriaShowsE(item.cat) && (
                    <div>
                      ...
                      <input type="number" className="form-input" min="0" value={item.ed} onChange={e => setItemField(bi, ii, 'ed', e.target.value)} placeholder="0" />
                    </div>
                  )}
                  {categoriaShowsS(item.cat) && (
                    <div>
                      <label className="form-label">Seleções (S)</label>
                      <input type="number" className="form-input" min="0" value={item.se} onChange={e => setItemField(bi, ii, 'se', e.target.value)} placeholder="0" />
                    </div>
                  )}
                </div>
                ...
                <div className="flex gap-2 items-center">
                  <input className="form-input" style={{ flex: 1 }} value={item.ob} onChange={e => setItemField(bi, ii, 'ob', e.target.value)} placeholder="Observação (opcional)" />
                  {ii > 0 && (
                    <button className="btn btn-ghost btn-icon" onClick={() => removeItem(bi, ii)} title="Remover" style={{ flexShrink: 0 }}>
